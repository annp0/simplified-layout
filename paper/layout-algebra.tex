...
every one of the \cuteCompSatisfied{} that satisfy the divisibility condition,
and fails for \cuteCompViolatedBroken{} of the \cuteCompViolated{} that do
not --- with no compile-time error, no assertion, and no run-time signal.
...
$A$ be disjoint. Of the \cuteCompTotal{} cases, the \cuteCompSatisfied{}
satisfying $N_0 d_0 \mid d_1$ all pass; of the \cuteCompViolated{} that do
not, \cuteCompViolatedBroken{} fail. The condition is therefore sufficient and
not necessary, which is exactly why its absence bites: most violations happen
to be harmless, nothing distinguishes those from the rest, and the ones that
are not harmless fail the codomain bound --- never orderedness, never
disjointness --- without a diagnostic. Measured against CUTLASS 4.8; the
