...
\paragraph{The two central results are machine-checked.} Lemma~\ref{lem:dense}
and Theorem~\ref{thm:decide} are proved in Coq, \evalCoqLines{} lines, each as
an \emph{if and only if}; no proof is admitted and \texttt{Print Assumptions}
reports every one closed under the global context. Two things are proved
...
same maps and return the same chain. Not mechanized: the separability of
Lemma~\ref{lem:separable}, which reduces the question to one axis; the
minimality of the coarsest chain, which the tests check against the
brute-force oracle; and the reordering of modes by stride, the proofs being
stated for modes already sorted.
