...
% paper/eval.sh from a run of the test suite. Nothing in the evaluation
% is a number typed by hand.
\input{eval-numbers}

% Figures measured against a CUTLASS checkout, written by study/run.sh.
\input{../study/cute-numbers}
...
layout as well. We show that when it can be written this way there is a
coarsest such form, whose digits every other one refines, and we emit that.
...
The decision procedure is machine-checked. Its two ingredients --- the
reduction to one coordinate at a time, and the linear-time recognition of a
layout on that coordinate --- are proved in Coq, each as an \emph{if and only
if}, with no axioms and no admitted goals; composing them gives the procedure
as a whole, so that the checks the implementation runs are proved to succeed
exactly when the map is a layout over a refinement of its domain. The proved
scan is extracted to OCaml and agrees with the implementation, verdict and
digits alike, on every map of the enumeration we test. We also measure the
system we are replacing: CuTe does not check its admissibility conditions in
general, and where they fail its complement can silently violate the
postconditions CuTe's own tests assert.

The implementation is roughly 1{,}000 lines of OCaml and the proofs roughly
2{,}000 of Coq.
\end{abstract}
...
None of these conditions appears in the CuTe API, and CUTLASS does not in
general check them. Composition's two divisibility conditions are written in
its source twice over: in a strong form that is commented out, and in a weak
form that runs only when both operands are statically known. The runtime
assertion for the dynamic case is commented out as well, and the reason is
given there --- it ``can cause extra registers and inappropriate
compile-time/run-time failure''. The complement's conditions are not checked
at all; its shape entries are integer quotients, and what is asserted is only
that they do not come out zero. The consequence is silence rather than a
diagnostic. Sweeping the \cuteCompTotal{} rank-2 layouts and cotargets of
Section~\ref{sec:eval}, all of them static, so this is the case CuTe
\emph{can} check, the complement meets its own documented postconditions for
every one of the \cuteCompSatisfied{} that satisfy both divisibility
conditions, and fails for \cuteCompViolatedBroken{} of the
\cuteCompViolated{} that do not --- with no compile-time error, no assertion,
and no run-time signal.
...
is \evalLibLines{} lines of OCaml, over \evalLibMliLines{} lines of
interface; the tests are \evalTestLines{}. Every figure quoted below is
generated from a run of the test suite (\texttt{paper/eval.sh}), not
transcribed: the random generators are seeded, so the numbers are
reproducible on the tree that produced them.

\paragraph{The decision procedure is correct.} The criterion of
Lemma~\ref{lem:dense} agrees with enumeration on \evalDenseRandom{} random
layouts. The scan of
...
\evalAxisFunctions{} functions, the two accept the same \evalAxisStrided{},
with the same chain.

\paragraph{CuTe's conditions are not checked, and the failure is silent.}
The sweep quoted in Section~\ref{sec:intro} takes every rank-2 layout
$(N_0,N_1){:}(d_0,d_1)$ with $N_0, N_1 \in \{2,3,4\}$, $d_0 \in \{1,2\}$ and
$d_1 \le 16$, against cotargets $M \in \{12,24,48\}$, keeping those for which
the complement formula is defined at all --- CuTe rejects $d_1 < N_0 d_0$ with
a \texttt{static\_assert}, the one condition it does enforce. Every entry is a
static integer, so nothing here is hidden from the compiler. Each complement is
checked against the postconditions CuTe's own unit test asserts: that
$\cosize(A, A^{*})$ reach $M$, that $A^{*}$ be ordered, and that $A^{*}$ and
$A$ be disjoint. Cases are classified by \emph{both} of the complement's
divisibility conditions, $N_0 d_0 \mid d_1$ and $N_1 d_1 \mid M$. Of the
\cuteCompTotal{} cases, the \cuteCompSatisfied{} satisfying both all pass; of
the \cuteCompViolated{} that violate one or the other,
\cuteCompViolatedBroken{} fail. The conditions are therefore sufficient and
not necessary, which is exactly why their absence bites: most violations
happen to be harmless, nothing distinguishes those from the rest, and the ones
that are not harmless fail the codomain bound --- never orderedness, never
disjointness --- without a diagnostic. The mechanism is plain in the formula:
its shape entries are quotients, so when a divisibility fails the integer
division truncates and the complement simply covers less than it should. For
$(2,2){:}(1,3)$ at $M = 24$ the entry $d_1 / N_0 d_0 = 3/2$ becomes $1$, and
$\cosize$ comes out $23$. Measured against CUTLASS 4.8; the
harness is \texttt{study/run.sh} and needs no GPU.

\paragraph{The decision procedure is machine-checked.} Lemma~\ref{lem:dense},
Lemma~\ref{lem:separable} and Theorem~\ref{thm:decide} are proved in Coq,
\evalCoqLines{} lines, each as an \emph{if and only if}; no proof is admitted
and \texttt{Print Assumptions} reports every one closed under the global
context. Composing them gives the statement about the procedure as a whole:
the checks of this section --- separability in one pass over the box, then the
scan of Theorem~\ref{thm:decide} on each axis --- succeed exactly when the map
is the index function of a layout over a refinement of its domain, plus a
constant. One direction is what stops a wrong address expression being
emitted; the other is what makes ``decided rather than searched'' true, since
it says no simplification is missed. Two things are proved
differently from the presentation above. The scan is phrased with its residual
recomputed from the weights committed so far rather than by mutating a copy of
the first difference: that is the invariant the proof of
Theorem~\ref{thm:decide} establishes anyway, so it is the same number at every
step. And the forward direction of Lemma~\ref{lem:dense} avoids the counting
step of its proof --- that modes $1,\dots,j{-}1$ produce at most $N_{j-1}$
values --- in favour of the observation that disjoint blocks of $n$ consecutive
positions covering $[0,M)$ must be the aligned ones, where the covering comes
from surjectivity and the disjointness from injectivity, so no cardinality
argument is needed. The proved scan and the implementation are not merely
alike: the scan is extracted from Coq to OCaml and run beside
\texttt{fit\_axis} on every one of the \evalExtractedMaps{} maps of the
enumeration above, where they agree on the verdict and, on the
\evalExtractedStrided{} they accept, digit for digit. The extraction is
faithful --- no datatype is remapped onto a machine integer --- so nothing is
assumed of it that the proofs do not establish. Not mechanized: the minimality of the
coarsest chain, which the tests check against the brute-force oracle; the
regrouping of a nested shape into the flat shape of its leaves; and the
reordering of modes by stride, the proofs being stated for modes already
sorted.

\paragraph{It matters in practice.} The tests build \evalLayouts{} layouts
--- CuTe's documented examples, CUTLASS's atoms, the usage table of
Section~\ref{sec:idioms}, and the laws of Section~\ref{sec:ops} --- and every
one of them is checked against its emitted expression at every coordinate. Of
those, \evalLayoutsAffine{} are affine in their coordinates,
\evalLayoutsRefined{} are layouts of a refinement with one coordinate split,
and \evalLayoutsPipeline{} are not layouts and are emitted as composites. On
\evalPairs{} random composites of a dense bijection
with a padded storage map, \evalPairsAffine{} are affine and
\evalPairsStrided{} are layouts of some refinement,
and their emitted forms contain \evalPairsOpsEmitted{} divisions and moduli
where the composites contain \evalPairsOpsPipeline{}. Chaining two logical
stages onto storage rather than one, \evalChainsStrided{} of \evalChains{} are
still layouts of some refinement, \evalChainsAffine{} of them affine: the
property survives composition. With a random swizzle on the storage map none of
\evalSwizzled{} is a layout, though all \evalSwizzledPlainStrided{} of the same
composites are without it --- the swizzle alone is what costs the strided
form.
...
\]
equals NVIDIA's printed layout at all \evalSmNinetyCoords{} coordinates. A GEMM mainloop
